...
export default function Home() {
    ...
    const [engineMode, setEngineMode] = useState('abstractive')
        ...
        try {
            ...
            const response = await fetch(API_URL, {
                ...
                body: JSON.stringify({
                    ...
                    method: engineMode,
                    groq_token: config.groqToken || null,
    ...
    const exportPDF = () => {
        ...
        const doc = new jsPDF()
        doc.setFontSize(16)
        doc.text("Thryve Summary", 10, 15)
        doc.setFontSize(11)
        const lines = doc.splitTextToSize(result.summary, 190)
        doc.text(lines, 10, 25)
        doc.save("thryve_summary.pdf")
    }

    const exportMarkdown = () => {
        if (!result?.summary) return
        const mdText = `# Thryve Summary\n\n> Method: ${result.method}\n> Compression: ${result.compression_ratio}%\n\n${result.summary}`
        const blob = new Blob([mdText], { type: 'text/markdown' })
        const url = URL.createObjectURL(blob)
        const a = document.createElement('a')
        a.href = url
        a.download = 'thryve_summary.md'
        a.click()
    ...
    return (
        <>
            ...
            <main className="glass-panel">
                ...
                <div style={{ marginTop: '20px', display: 'flex', gap: '16px', background: 'rgba(0,0,0,0.2)', padding: '12px 16px', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.05)' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flex: 1 }}>
                        <div style={{ display: 'flex', flexDirection: 'column' }}>
                            <span style={{ fontFamily: 'var(--font-display)', fontSize: '13px', fontWeight: 600, color: engineMode === 'abstractive' ? 'rgba(255,255,255,0.3)' : 'rgba(255,255,255,0.8)' }}>
                                Extractive
                            </span>
                            <span style={{ fontFamily: 'var(--font-display)', fontSize: '10px', color: 'rgba(255,255,255,0.25)', letterSpacing: '0.5px' }}>
                                copies key sentences
                            </span>
                        </div>
                        <div
                            role="switch"
                            aria-checked={engineMode === 'abstractive'}
                            aria-label="Toggle abstractive AI mode"
                            tabIndex={0}
                            onClick={() => setEngineMode(engineMode === 'extractive' ? 'abstractive' : 'extractive')}
                            onKeyDown={e => (e.key === ' ' || e.key === 'Enter') && setEngineMode(engineMode === 'extractive' ? 'abstractive' : 'extractive')}
                            style={{
                                width: '44px', height: '26px',
                                background: engineMode === 'abstractive' ? 'var(--neon-purple)' : 'rgba(255,255,255,0.15)',
                                borderRadius: '13px', position: 'relative', cursor: 'pointer',
                                transition: 'background 0.25s', flexShrink: 0, outline: 'none',
                                boxShadow: engineMode === 'abstractive' ? '0 0 14px rgba(140,0,255,0.5)' : 'none'
                            }}
                        >
                            <div style={{
                                width: '18px', height: '18px', background: '#fff', borderRadius: '50%',
                                position: 'absolute', top: '4px',
                                left: engineMode === 'abstractive' ? '22px' : '4px', transition: 'left 0.25s ease',
                            }} />
                        </div>
                        <div style={{ display: 'flex', flexDirection: 'column' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginTop: '40px' }}>
                    ...
                    {warmupStatus && loading && (
                        <div style={{ textAlign: 'center', color: 'var(--neon-cyan)', fontSize: '12px', fontFamily: 'var(--font-display)' }}>
                            {warmupStatus}
                        </div>
                    )}
                </div>
                    ...
                    {config.showStats && (
                        <div className="stats-four-col" style={{ marginBottom: '28px' }}>
                            {[
                                ...
                                { label: 'ROUGE-L Score', val: result.rouge_l ? result.rouge_l.toFixed(3) : '—' },
                            ].map((s, i) => (
                                <div key={i} className="stat-box">
                                    <div className="stat-label">{s.label}</div>
                                    <div className="stat-val" style={{ fontSize: '24px', color: i === 3 ? 'var(--neon-pink)' : 'inherit' }}>{s.val}</div>
                                </div>
                            ))}
                        </div>
                    )}
                    ...
                    {activeTab === 'summary' && (
                        <>
                            <div className="result-header">
                                {/* Dynamic title based on which engine was used */}
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                                    ...
                                    <span style={{
                                        ...
                                        color: result.method?.startsWith('abstractive')
                                ...
                                <div style={{ display: 'flex', gap: '8px' }}>
                                    <button className="btn-cyber-ghost-sm" onClick={exportMarkdown} style={{ flex: 0, padding: '6px 12px', whiteSpace: 'nowrap' }}>
                                        Export MD
                                    </button>
                                    <button className="btn-cyber-ghost-sm" onClick={exportPDF} style={{ flex: 0, padding: '6px 12px', whiteSpace: 'nowrap' }}>
                                        Export PDF
                                    </button>
                            ...
                            {result.method?.startsWith('abstractive') ? (
                                /* ── Abstractive: flowing paragraph output ── */
                                <div style={{
                                    marginTop: '16px',
                                    padding: '24px 28px',
                                    background: 'linear-gradient(135deg, rgba(140,0,255,0.04), rgba(0,0,0,0.2))',
                                    borderRadius: '14px',
                                    border: '1px solid rgba(140,0,255,0.18)',
                                    boxShadow: '0 0 40px rgba(140,0,255,0.04) inset',
                                }}>
                                    {result.summary.split('\n').filter(p => p.trim()).map((para, i) => (
                                        <p key={i} style={{
                                            fontFamily: 'var(--font-body)',
                                            color: 'rgba(255,255,255,0.90)',
                                            fontSize: config.fontSize === 'small' ? '14px' : config.fontSize === 'large' ? '18px' : '15.5px',
                                            lineHeight: config.fontSize === 'small' ? 1.7 : config.fontSize === 'large' ? 2.0 : 1.85,
                                            marginBottom: i < result.summary.split('\n').filter(p => p.trim()).length - 1 ? '16px' : 0,
                                            margin: i < result.summary.split('\n').filter(p => p.trim()).length - 1 ? '0 0 16px 0' : 0,
                                        }}>
                                            {para}
                                        </p>
                                    ))}
